(ns beverageops.operation
  ...
  (BeverageServiceAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the BeverageServiceGovernor
  ...
  human operator/venue manager. The approver resumes with
...
(defn build
  ...
  `beverageops.store/Store`).
  opts:
    :advisor      -- a `beverageops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; BeverageServiceAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; BeverageServiceGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
            ...
            (case disposition
              ...
              {:disposition :escalate
               :audit [{:t :approval-requested
                        :op (:op request) :venue-id (:venue-id request)
                        :reason (or reason
          ...
          (if (= :approved (:status approval))
            {:disposition :commit
             ...
             :audit [{:t :approval-granted :op (:op request)
                      :venue-id (:venue-id request) :by (:by approval)}]}
            {:disposition :hold
